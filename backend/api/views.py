...
class BudgetViewSet(viewsets.ModelViewSet):
    ...
    def summary(self, request):
        """
        Get overall budget summary for current month
        ...
        total_limit = budgets.aggregate(Sum('monthly_limit'))['monthly_limit__sum'] or Decimal('0.00')
...
class AnalyticsViewSet(viewsets.ViewSet):
    ...
    def monthly_summary(self, request):
        ...
        total_budget = Budget.objects.filter(
            user=request.user,
            is_active=True
        ).aggregate(Sum('monthly_limit'))['monthly_limit__sum'] or Decimal('0.00')
